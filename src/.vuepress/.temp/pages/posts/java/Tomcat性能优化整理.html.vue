...
<p>​		maxThreads Tomcat 使用线程来处理接收的每个请求。这个值表示Tomcat可创建的最大的线程数。
​		acceptCount指定当所有可以使用的处理请求的线程数都被使用时，可以放到处理队列中的请求数，超过这个数的请求将不予处理。
​		connnection Timeout网络连接超时，单位:毫秒。设置为0表示永不超时，这样设置有隐患的。通常可设置为30000毫秒。</p>
<p>​		minSpareThreadsTomcat初始化时创建的线程数。
...
<p>​		可以根据自己机器的内存进行更改。</p>
<p>（2）java-Xms32m-Xmx800m className
...
<p>（3）java.lang.OutOfMemoryError: PermGen space
...
<p>（4）java.lang.OutOfMemoryError:Java heap space Heap size设置
...
<p>​		提示:在JVM中如果98%的时间是用于GC且可用的Heap size不足2%的时候将抛出此异常信息。
