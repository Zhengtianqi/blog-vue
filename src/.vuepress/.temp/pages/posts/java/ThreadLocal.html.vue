<template><div><p>引言：本博客《SimpleDateFormat引发的线程安全问题》中提到，可以利用 ThreadLocal 来解决SimpleDateFormat的线程安全问题。之后看到阿里巴巴开发规范中也有提到，SimpleDateFormat禁止使用static进行修饰。</p>
<figure><img src="/assets/images/simpleDateFormat-alibaba.png" alt="image-20200331153816952" tabindex="0" loading="lazy"><figcaption>image-20200331153816952</figcaption></figure>
<h1 id="一、threadlocal用在什么地方" tabindex="-1"><a class="header-anchor" href="#一、threadlocal用在什么地方" aria-hidden="true">#</a> 一、ThreadLocal用在什么地方？</h1>
...
<h1 id="二、用一下才知道它能干什么" tabindex="-1"><a class="header-anchor" href="#二、用一下才知道它能干什么" aria-hidden="true">#</a> 二、用一下才知道它能干什么！</h1>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>threadlocal1<span class="token operator">=</span><span class="token number">0</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>结果可以看到 ：</p>
<p>（1）第二个线程 是访问不到 第一个线程 所存的值的。它们存在线程的隔离。</p>
<p>（2）也就是说每个线程有一个自己的 ThreadLocalMap ，所以每个线程往这个 ThreadLocal 中读写隔离的，并且是互相不会影响的。</p>
<h1 id="三、看一下它的存储" tabindex="-1"><a class="header-anchor" href="#三、看一下它的存储" aria-hidden="true">#</a> 三、看一下它的存储</h1>
<figure><img src="/assets/images/ThreadLocal内部存储.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>一个ThreadLocal只能存储一个Object对象，如果需要存储多个 Object 对象那么就需要多个 ThreadLocal</p>
<h1 id="四、内存泄露" tabindex="-1"><a class="header-anchor" href="#四、内存泄露" aria-hidden="true">#</a> 四、内存泄露？</h1>
...
<p>学习资源来自于： <a href="http://www.jiangxinlingdu.com/" target="_blank" rel="noopener noreferrer">http://www.jiangxinlingdu.com/<ExternalLinkIcon/></a> 「公众号：匠心零度 」中的《手撕面试题threadlocal》</p>
</div></template>
